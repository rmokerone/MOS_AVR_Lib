...
int main (void)
{
    ...
    MOS_SPI_Init(spi_init_instruct);
    data = MOS_SPI_Master_WriteRead(spi_init_instruct,
                                    data, SPI_CSN_ASSERTED);
    data = MOS_SPI_Master_Write(spi_init_instruct,
                                    data);
    data = MOS_SPI_Master_Read(spi_init_instruct);
    return 0;
}
